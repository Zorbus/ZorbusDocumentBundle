...
class BlockTagConfig extends BlockConfig
{

    public function __construct(AdminInterface $admin, FormFactory $formFactory, $httpKernel)
    {
        parent::__construct('zorbus_document.block.tag', 'Tag Block', $admin, $formFactory);
        $this->enabled = true;
        $this->themes = array(
            'ZorbusDocumentBundle:Block:tag' => 'Default'
            );
    ...
    public function getBlockEntity(array $data, BlockEntity $block = null)
    {
        ...
        $block->setParameters(json_encode(array('tag_id' => $data['tag']->getId())));
