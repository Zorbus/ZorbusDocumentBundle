...
namespace Zorbus\DocumentBundle\Model;
...
class BlockDocumentConfig extends BlockConfig
{

    public function __construct(AdminInterface $admin, FormFactory $formFactory, $httpKernel)
    {
        parent::__construct('zorbus_document.block.document', 'Document Block', $admin, $formFactory);
        $this->enabled = true;
        $this->themes = array(
            'ZorbusDocumentBundle:Block:document' => 'Default'
            );
    ...
    public function getFormMapper()
    {
        return $this->formMapper
                        ->add('document', 'entity', array(
                            'class' => 'Zorbus\\DocumentBundle\\Entity\\Document',
    ...
    public function getBlockEntity(array $data, BlockEntity $block = null)
    {
        ...
        $block->setCategory('Document');
        $block->setParameters(json_encode(array('document_id' => $data['document']->getId())));
